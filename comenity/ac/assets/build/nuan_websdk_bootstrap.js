	...
	*/
	...
	function setDomReadyTimeout(t) {
		setTimeout(function(){
			...
			div.style.cssText = "z-index:99;height:1px;width:1px;" +
...
FrameCommunicationHandler.prototype.receiveMessage = function(e){
	...
	var dataObj;
	try {
		var dataObj = typeof e.data === 'string' ? JSON.parse(e.data) : e.data;
		if(this.verifyMessage(dataObj)) {
			...
			switch(dataObj.command) {
				...
				*/
				...
				const acURL = (window.location.href).split("ac/");
				const newURL = acURL[1].split("/");
				const brand =  newURL[0];

				if(window.location.href == domain+'/ac/'+brand+'/public/home' && dataObj.info.data.customerMsgCount > 0){
					if(messageWindow) {
		...
		if (!e.data || typeof e.data !== 'string' || e.data.indexOf('iframeloadsuccess') < 0) {
			// console.log(e);
